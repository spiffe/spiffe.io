...
This example of creating secrets uses a bootstrap cert and key, as configured in the **server-secrets.yaml** configuration file. For production deployments the default values provided here should *never* be used.

The value for `bootstrap.key` is a base64 encoding of dummy_upstream_ca.key, generated by running the following base64 command:
