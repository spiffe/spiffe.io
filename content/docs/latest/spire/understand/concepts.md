...
13. The server sees the SPIFFE ID of the agent's own SVID (the one used as its TLS client certificate), and fetches all [authorized registration entries](#authorized-registration-entries). All those are the workload entries signed and return to the agent.
